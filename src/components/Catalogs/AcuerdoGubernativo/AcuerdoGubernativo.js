...
import {
    getListaAcuerdoGubernativo,
    crearAcuerdoGubernativo,
...
const AcuerdoGubernativo = () => {
    const [data, setData] = useState([]);
    const [filteredData, setFilteredData] = useState([]);
    const [searchText, setSearchText] = useState('');
    const [showModal, setShowModal] = useState(false);
    const [selectedAcuerdoGubernativo, setSelectedAcuerdoGubernativo] = useState(null);
    ...
    const fetchData = async () => {
        try {
            const response = await getListaAcuerdoGubernativo();
            setData(response.data);
            setFilteredData(response.data);
        } catch (error) {
            console.error('Error al obtener acuerdoGubernativos:', error);
        }
    };

    const handleOpenModal = (acuerdoGubernativoData = null) => {
        setSelectedAcuerdoGubernativo(acuerdoGubernativoData);
        setShowModal(true);
    };
    ...
    const handleToggleEstado = async (row) => {
        try {
            ...
            EstadoAcuerdoGubernativo(row.id, { activo: nuevoEstado })
    ...
    const confirmDelete = async (id, toastId) => {
        try {
            await EliminarAcuerdoGubernativo(id);
    ...
    const handleCloseModal = () => {
        setSelectedAcuerdoGubernativo(null);
        setShowModal(false);
    };

    const handleSubmitColaborador = async (acuerdoGubernativoData, idAcuerdoGubernativo) => {
        try {
            const { numero } = acuerdoGubernativoData;
            const dataToSend = { numero };
            
            if (idAcuerdoGubernativo) {
                await EditarAcuerdoGubernativo(idAcuerdoGubernativo, dataToSend);
                ...
                await crearAcuerdoGubernativo(dataToSend);
            ...
            console.error('Error al guardar acuerdoGubernativoData:', error);
            ...
            <div>
                ...
                >
                    ...
                    <ModalForm
                        ...
                        idAcuerdoGubernativo={selectedAcuerdoGubernativo?.id || null}
                        acuerdoGubernativoData={selectedAcuerdoGubernativo}
...
export default AcuerdoGubernativo;
